...
public class RFIDNotaActivity extends Activity {
...
//	public static String NOMETABELA;
	...
	private Button btn_buscarProdutos;
	private EditText etNumeroNota;
	private String url;
	...
	private Runnable exibeUiErroException(final Exception exception, final String msg){
		...
		return new Runnable() {						
					...
					public void run() {					
							...
							new AlertDialog.Builder(RFIDNotaActivity.this)
							.setTitle("Conex�o")
							.setMessage(msg)
	...
	public void onCreate(Bundle savedInstanceState){
		...
		setContentView(R.layout.rfidprodutosnota);		
		...
		btn_buscarProdutos  = (Button) findViewById(R.id.btn_buscarProdutos);
		etNumeroNota	    = (EditText) findViewById(R.id.etNumeroNota);
		dh = new DataBaseHelper(RFIDNotaActivity.this);
		try {
			rfidController = new RFIDController(RFIDNotaActivity.this);
				...
				try{
					runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							tvImportando.setVisibility(View.VISIBLE);
						}
					});
					resposta = null;
					urlRequestProdutos = url + Parametro.actionRequestProdutosNota;
                    urlRequestProdutos += "/" + etNumeroNota.getText();


					//verifica a url � valida
					...
					try{
							...
							public void run() {
								...
								if(parametro.getJsonFromBD() != null && !parametro.getJsonFromBD().equals("null")){
									try {
										...
										adapterProdutos = new SimpleAdapter(RFIDNotaActivity.this ,rfidprodutos,
										...
										btn_buscarProdutos.setVisibility(View.GONE);
										etNumeroNota.setVisibility(View.GONE);
										btn_inventario.setVisibility(View.VISIBLE);
                                        lvRFIDProdutos.setVisibility(View.VISIBLE);

										Toast.makeText(RFIDNotaActivity.this, String.valueOf(jaArray.length()) + " produtos importados", Toast.LENGTH_SHORT).show();
						...
						runOnUiThread(exibeUiErroException(ex, "N�o foi poss�vel conectar ao servidor, o servidor n�o respondeu"));
						
					}
				}catch(MalformedURLException e){					     
						final MalformedURLException ex = e;
					    runOnUiThread(exibeUiErroException(ex, "Erro na sintaxe da URL ou na conex�o com o servidor. msg: " + ex.getMessage()));
	...
	public void chamaInventario(View view){
		...
		Intent intent = new Intent(RFIDNotaActivity.this, RFIDHostActivity.class);
		RFIDHostActivity.LIST_RFIDPRODUTOS = rfidprodutos;
		RFIDHostActivity.nota = true;
